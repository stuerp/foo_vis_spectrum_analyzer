...
void diff(std::vector<float> in, std::vector<float> & out)
{
    ...
    for (int i = 1; i < in.size(); ++i) out[i - 1] = in[i] - in[i - 1];
}
...
{
    for (int indice : indices) out.push_back(in[indice]);
}

void selectElementsFromIndices(std::vector<int> in, const std::vector<int> & indices, std::vector<int> & out)
{
    for (int indice : indices) out.push_back(in[indice]);
}
...
void scalarProduct(float scalar, std::vector<float> in, std::vector<float> & out)
{
    ...
    for (int i = 0; i < in.size(); ++i) out[i] = scalar * in[i];
}

void PeakFinder::findPeaks(std::vector<float> x0, std::vector<int> & peakInds, bool includeEndpoints, float extrema)
{
    int minIdx = static_cast<int>(distance(x0.begin(), min_element(x0.begin(), x0.end())));
    int maxIdx = static_cast<int>(distance(x0.begin(), max_element(x0.begin(), x0.end())));
    ...
    if (includeEndpoints)
    {
        ...
        ind.insert(ind.end(), static_cast<int>(len0));

        minMagIdx = static_cast<int>(distance(x.begin(), std::min_element(x.begin(), x.end())));
        minMag = x[minMagIdx];
        leftMin = minMag;
    }
    else
    ...
    if (includeEndpoints)
    {
        // Deal with first point a little differently since tacked it on. Calculate the sign of the derivative since we tacked the first point on it does not necessarily alternate like the rest.
    ...
    if (x[0] >= x[1]) ii = 0; else ii = 1;

    //Preallocate max number of maxima
    float maxPeaks = ceil((float) len / 2.0);
    std::vector<int> peakLoc(static_cast<size_t>(maxPeaks), 0);
    std::vector<float> peakMag(static_cast<size_t>(maxPeaks), 0.0);
    int cInd = 1;
    int tempLoc;
    ...
    while (ii < len)
    {
        ...
        // Found new peak that was larger than temp mag and selectivity larger /than the minimum to its left.
        ...
        if (x[ii - 1] < leftMin) leftMin = x[ii - 1]; // New left minima
    }
